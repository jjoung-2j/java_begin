	...
	public static void main(String[] args) {
		...
		Functional_Area_1 area_obj = new Functional_Area_1() {
			...
			public void area(double x, double y, int type) {
				...
				case 2:
					...
					break;
				case 3:
					System.out.println("장축의 반지름 " + x + ", 단축의 반지름" + y + "인 타원형의 면적은 : " + ((3.141592)*x*y));
					break;
				case 4:
	   ...
	   */
		...
		Functional_Search_4 lambda_srch = (str) -> System.out.println("회원들 중 \"" + str + "\"님을 조회합니다.");
			// 메소드 생략, 파라미터 타입 생략 => str 	// { } 안 명령문이 1개이라면 생략
		lambda_srch.search("이순신");
		// 회원들 중 "이순신"님을 조회합니다.
		
		// 4. 파라미터의 개수가 1개일 때는 파라미터를 감싸는 소괄호() 를 생략할 수 있다.
		
		Functional_Search_4 lambda_srch2 = str -> System.out.println("회원들 중 '" + str + "'님을 조회합니다.");
			// 메소드 생략, 파라미터 타입 생략, 파라미터 개수 1개이므로 소괄호 생략 => str 	// { } 안 명령문이 1개이라면 생략
		lambda_srch2.search("엄정화");
		// 회원들 중 "엄정화"님을 조회합니다.
		
		// 5. 파라미터의 개수가 2개 이상일 때는 무조건 괄호 ()를 꼭 기재해야 한다.
	/*
		Functional_Operator_3 annonymous_op4 = a, b, type -> {		// () 가 없으므로 오류 발생함.
