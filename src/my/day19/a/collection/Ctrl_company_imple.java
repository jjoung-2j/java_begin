...
import java.util.Scanner;
	...
	//	List<Company> cp_list, Map<String, Company> cp_map 에 저장시켜주는 메소드 생성하기 ==
	...
	public void register(Scanner sc, List<Company_imple> cp_list, Map<String, Company_imple> cp_map) {
			...
			do {
				...
				// == 중복 아이디 검사하기 시작 == //
				if(cp_map.get(id) != null) {	// 아이디가 이미 존재하는 경우
	...
	public Company_imple login(Scanner sc, Map<String, Company_imple> cp_map) {
		...
		if(cp_map.get(id) != null && passwd.equals(cp_map.get(id).getPasswd())) {	// key 값 : id != null
			return cp_map.get(id);		// 구인회사 정보 들어갈 것
		} else 
			return null;	// 로그인 실패		
	}	// end of public CommonMember login(Scanner sc, Map<String, Company_imple> cp_map)------------
	...
	do {
		System.out.println("=== 구인회사 전용메뉴(" + login_cp.getName() + "기업 로그인 중) ===\n"
						...
						+ "6. 구직자 연령대 및 성별 검색 7. 채용공고 입력하기 8. 우리회사 채용공고 조회  9. 우리회사 채용공고 지원자 10. 로그아웃\n");
...
}	// end of public void cp_menu(Scanner sc, Company login_cp, List<Gujikja_imple> gu_list, List<Recruit_imple> rc_list, List<RecruitApply> rcapply_list------

	// // 회사정보 보기
	private void view_myInfo_company(Company_imple login_cp) {
	...
	// 회사정보 수정
	private void update_myInfo_company(Scanner sc, Company_imple login_cp) {
		view_myInfo_company(login_cp);
	...
	}	// end of private void view_all_gujikja_info(List<Gujikja_imple> gu_list)------------------
	...
	// 구직자 성별 검색
	private void search_gender(Scanner sc, List<Gujikja_imple> gu_list) {
		
		int cnt = 0;
		for(int i=0; i<gu_list.size(); i++) {
			if(gu_list.get(i).getType() == 1) {
			// 또는
			// if(cmbr_arr[i] instanceof Gujikja)
				cnt++;
			}
		}	// end of for----------------
		if(cnt == 0) {	// 구직자가 존재하지 않는 경우
			...
			for(int i=0; i<gu_list.size(); i++) {
				 ...
				 if( gu_list.get(i) instanceof Gujikja_imple && 
						 input_gender.equals(gu_list.get(i).getGender()))  {
					isSearch = true;
					sb.append(gu_list.get(i).getInfo() + "\n");
				}
...
}	// end of private void search_gendersearch_gender(Scanner sc, List<Gujikja_imple> gu_list)-------
	
	// 구직자 연령대 검색
	private void search_ageLine(Scanner sc, List<Gujikja_imple> gu_list) {
		
		if(gu_list.size() == 0) {	// 구직자가 존재하지 않는 경우
			System.out.println(">> 구직자로 가입된 회원이 아무도 없습니다. <<\n");
		} else {	// 구직자가 존재하는 경우
			...
			for(int i=0; i<gu_list.size(); i++) {
				if(gu_list.get(i) instanceof Gujikja_imple) {
					int ageLine = gu_list.get(i).getAge()/10*10;
									// 나이/10*10
									// 26/10*10 ==> 20
									// 23/10*10 ==> 20
	...
	}	// end of private void search_ageLine(Scanner sc, List<Gujikja_imple> gu_list)-----
	...
	// 구직자 연령대 및 성별 검색
	private void search_ageLine_gender(Scanner sc, List<Gujikja_imple> gu_list) {
		...
		if(gu_list.size() == 0) { // 구직자가 없는 경우
			...
			if(!is_ageline_search) { // 검색하고자 하는 연령대에 해당하는 구직자가 존재하지 않는 경우 
				...
				for(Gujikja_imple gu : gu_list) {
					if( Integer.parseInt(str_ageLine) == gu.getAge()/10*10   
					    && input_gender.equals( gu.getGender()) ) {
	...
	}	// end of private void search_ageLine_gender(Scanner sc, List<Gujikja_imple> gu_list)------
	...
	private void register_recruit(Scanner sc, Company_imple login_cp, List<Recruit_imple> rc_list) {
		...
		Recruit_imple rc = new Recruit_imple();		// rc 에 채용공고 등록
		
		do {
			System.out.print("1. 채용제목 : ");
			String subject = sc.nextLine();
			rc.setSubject(subject);
		} while (!(rc.getSubject() != null));	// null 이 아니라는 것은 올바르게 들어갔다는 말이다.
		
		do {
			System.out.print("2. 채용분야[예> 사무직] : ");
			String work_type = sc.nextLine();
			rc.setWork_type(work_type);
		} while (!(rc.getWork_type() != null));	// null 이 아니라는 것은 올바르게 들어갔다는 말이다.
		...
		} while (!(rc.getCnt() > 0));	// 입력이 되지 않은 경우, 초기값이 0 이기 때문에 0 보다 커야한다.
		...
		} while (!(rc.getYearpay() > 0));	// 입력이 되지 않은 경우, 초기값이 0 이기 때문에 0 보다 커야한다.
		
		do {
			System.out.print("5. 채용마감일자[예{> 20240208] : ");
			String finish_day = sc.nextLine();
			rc.setFinish_day(finish_day);
		} while (!(rc.getFinish_day() != null));	// null 이 아니라는 것은 올바르게 들어갔다는 말이다.
	...
	}	// end of private void register_recruit(Scanner sc, Company_imple login_cp, List<Recruit_imple> rc_list)-----------
	...
	private void view_recruit_mycompany(Company_imple login_cp, List<Recruit_imple> rc_list) {
		...
		for(Recruit_imple rc : rc_list) {
			if(rc.getCp().getId().equals(login_cp.getId())){
				is_existence = true;	// 동일한 것을 발견했을 경우
				System.out.println(rc.recruit_info()); 			// 채용정보를 보여주는 메소드
			}
		}	// end of for----------------
		
		if(!is_existence)
			System.out.println(">> 채용공고를 낸 것이 없습니다.<<\n");
	}	// end of private void view_recruit_mycompany(Company_imple login_cp, List<Recruit_imple> rc_list)---------
	...
	private void view_gujikja_my_recruitApply(Company_imple login_cp, List<RecruitApply> rcapply_list) {
		...
		for(int i=0; i<rcapply_list.size(); i++) {
			if(login_cp.getId().equals(rcapply_list.get(i).getRc().getCp().getId()))	{	// 채용지원 -> 어디 지원하는 지(채용정보) -> 회사 -> 회사 아이디
				is_existence = true;	// 지원자가 있을 경우
				sb.append(rcapply_list.get(i).getRc().getRecruit_no() + "\t");	// 채용공고번호
				sb.append(rcapply_list.get(i).getRc().getSubject() + "\t");		// 채용제목
				sb.append(rcapply_list.get(i).getGu().getName() + "\t");			// 지원자명
				sb.append(rcapply_list.get(i).getGu().getGender() + "\t");		// 성별
				sb.append(rcapply_list.get(i).getGu().getAge() + "\t");			// 나이
				sb.append(rcapply_list.get(i).getApply_motive() + "\n");			// 지원동기
			}
		}	// end of for-----------
		if(is_existence) {
	...
	}	// end of private void view_gujikja_my_recruitApply(Company_imple login_cp, List<RecruitApply> rcapply_list)--------
