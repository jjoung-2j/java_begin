...
public class Ctrl_common_imple implements Ctrl_common {
		...
		public CommonMember login(Scanner sc, CommonMember[] cmbr_arr, int type) {
			...
			for(int i=0; i<CommonMember.count; i++) {
				if(cmbr_arr[i].getType() == type && cmbr_arr[i].getId().equals(id) && cmbr_arr[i].getPasswd().equals(passwd))
						return cmbr_arr[i];
			}	// end of for-----------
			
			return null;	// 로그인 실패
		}	// end of public CommonMember login(Scanner sc, CommonMember[] cmbr_arr, int i)---------
}
