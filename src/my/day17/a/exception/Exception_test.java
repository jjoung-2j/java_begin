	...
	public static void main(String[] args) {
		...
		for(int i=3; i>=0; i--) {
			try {
				int val = num_arr[i]/i;
				System.out.println(val);
			} catch(Exception e) {		// ArrayIndexOutOfBoundsException 와 ArithmeticException 의 부모 Exception => 다형성
				System.out.println("에러메세지 : " + e.getMessage());
		}	// end of try~catch-----------------------
		}	// end of for-----------
		...
		for(int i=3; i>=0; i--) {
			try {
				int val = Integer.parseInt(str_arr[i])/i;	// (str_arr[3])/3 => 3번째가 없다. => ArrayIndexOutOfBoundsException
															// (str_arr[2])/2 => 가능
															// (str_arr[1])/1 => numberformatexception
															// (str_arr[0])/0 
				System.out.println(val);
			} catch(ArrayIndexOutOfBoundsException e) {		
				System.out.println(">> 배열의 인덱스 범위가 초과되었습니다. <<");
