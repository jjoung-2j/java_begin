...
public class Main_for_2_break_continue {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < 10; i++) {
			if((i+1)%2 == 0) {	// 출력하고자 하는 값이 짝수이라면
				continue;	// 반복문에서 continue; 를 만나면 실행순서가 밑으로 내려가지 않고 가장 가까운 반복문의 증감식으로 이동시켜준다.
							// continue 가 나오면 아래 실행하지 않고 증감식으로 이동한다.
			}
			System.out.print((i+1) + " ");
			// i ==> 0 (i+1) ==> 1	
			// i ==> 1 (i+1) ==> 2 continue; i++	// 밑으로 내려가지 않으니 (System.out.print 와 만나지 않아) 출력 X
			// i ==> 2 (i+1) ==> 3
		...
		for(int i = 0; i < 10; i++) {
			...
			/*
			if(i<9) {
				System.out.print((i+1) + ",");	// i<9 안할시, 1,3,5,7,9,
			}
			else {
				System.out.print((i+1));
			}
