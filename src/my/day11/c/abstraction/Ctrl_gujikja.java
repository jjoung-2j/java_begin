	...
	void register(Scanner sc, Gujikja[] gu_arr) {
		...
		if(Gujikja.count < gu_arr.length) {	// 지금까지 생성된 구직자 회원수가 gu_arr.length(==>정원) 보다 적을 경우에만 신규회원을 받아들인다.
			...
			do {
				...
				// 반복할때 false인 상태이니 true 로 바꾸어주기!	// 안바꾸어주면 처음에 못빠져나가면 빠져나갈수없다.
				System.out.print("1.아이디 : ");
				userid = sc.nextLine();	// "eomjh" "leess" "chaew" 현재 사용중인 아이디 이므로 입력불가!!
												// "" 또는 "        " 공백만으로는 입력불가!!
				// == 가입된 회원들에 대해 중복아이디 검사하기 ==
				// 1. for(int i=0; i<Gujikja.count; i++) {
		...
		do {
			...
			passwd = sc.nextLine();	// 틀린예시 "Qw12$"  "Qwer1234sdfsdfdsfsfsdf$"  "qwer1234$"  "qWer1234$" 
		...
		do {
			...
			name = sc.nextLine();	//"" "    "  "강 감 찬" "김수한무거북이두루미"
											//"강감찬"
			/* 내 방법
			if(2<=name.length() && name.length()<=6) {
				...
				isUse_name = false;
				System.out.println("[경고] 2글자 이상 6글자 이하 공백없는 한글로 입력해주세요.");
			}	// end of if-------------------------------
			*/
			// 강사님
			 ...
			 */
		} while(!isUse_name);	// end of do~while------------------------
		...
		do {
			...
			jubun = sc.nextLine();	// "9610022" "9610022"
									// "0010023" "0010024"
									// "9604311" "9604312" "0004314"	<- 오류사항
									// "9610025" "0010025" <- 원래있는데 비정상으로 볼것!
