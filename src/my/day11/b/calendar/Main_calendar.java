...
public class Main_calendar {
	...
	public static void main(String[] args) {
		...
		// >>> 현재시각 1 => Tue Jan 30 12:27:52 GMT+09:00 2024
		
		SimpleDateFormat sdformat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		System.out.println(">>> 현재시각 2 => " + sdformat.format(now));
		// >>> 현재시각 2 => 2024-01-30 12:29:16
		...
		/*
	      ...
	      */
		String currentTime = String.format("%tF", now);				// %t : time
		System.out.println(">>> 현재시각 3 => " + currentTime);
		// >>> 현재시각 3 => 2024-01-30
	
		String currentTime1 = String.format("%tF %tp %tT %tA %tZ", now, now, now, now, now);				// %t : time
		System.out.println(">>> 현재시각 4 => " + currentTime1);
		// >>> 현재시각 4 => 2024-01-30 오후 12:34:42 화요일 GMT+09:00		// 그리니치 표준시(협정세계시)보다 9시간 빠르다.
		...
		// === 문자열을 날짜 형태로 변환하기 === //
		try {
			Date d = sdformat.parse("19980430");	// 가능
			// Date d = sdformat.parse("19980431");	// 존재 O
		...
		/* == 2개의 날짜를 비교하여 2개의 날짜가 같은 날짜인지 아니면 2개의 날짜중 어떤 날짜가 이전날짜 인지 이후날짜인지 알아오기 == */
		try {
		      ...
		      /* 
		         >> Date 클래스에서 제공하는 compareTo() 메소드 <<
		            ...
		            date1.compareTo(date2);
		              ...
		              date1 날짜와 date2 날짜가 동일하면 0을 반환하고,								// date1 = date2
		              date1 날짜가 date2 날짜 보다 이전 날짜인 경우 0보다 작은 값을 반환하고, 			// date1 < date2(미래)	-> 0보다 작은값
		              반대로 date1 날짜가 date2 날짜 보다 이후 날짜인 경우 경우 0보다 큰 값을 반환한다. 	// date1 > date2(과거)	-> 0보다 큰값
		      */
		...
		System.out.println("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");   
		   ...
		   */   
		   ...
		   int month = currentDate.get(Calendar.MONTH); // 0 ~ 11 	주의 0이 1월 11이 12월
		   ...
		   */
		   ...
		   int month2 = currentDate.get(Calendar.MONTH)+1; // 1 ~ 12	//+1 해줘야만 1~12까지 표출됨
	    ...
	    System.out.println("이 달의 몇 일: " + currentDate.get(Calendar.DATE));		// 오늘이 2월1일이면
	    System.out.println("이 달의 몇 일: " + currentDate.get(Calendar.DAY_OF_MONTH));	//
	    System.out.println("이 달의 몇 일: " + strDay);
	    System.out.println("이 해의 몇 일: " + currentDate.get(Calendar.DAY_OF_YEAR));	// 32일
	    
	 // 1:일요일, 2:월요일, 3:화요일, 4:수요일, 5:목요일, 6:금요일, 7:토요일
	    System.out.println("요일(1~7, 1:일요일): " + currentDate.get(Calendar.DAY_OF_WEEK)); 		// 화요일 -> 2출력이 됨
	    System.out.println("이 달의 몇 째 주: " + currentDate.get(Calendar.DAY_OF_WEEK_IN_MONTH));	// 5번째 주-> 5출력이 됨
	    ...
	    System.out.println("오늘은 " + dayname + "요일 입니다.");		// 숫자를 한글로 바꾸기 위해 switch 문 사용
	    // 오늘은 화요일 입니다.
	    
	    System.out.println("오전_오후(0:오전, 1:오후): " + currentDate.get(Calendar.AM_PM));	// 오전 오후
	    System.out.println("시간(0~11): " + currentDate.get(Calendar.HOUR));					// 12시간
	    System.out.println("시간(0~23): " + currentDate.get(Calendar.HOUR_OF_DAY));			// 24시간
	    ...
	    System.out.println("TimeZone(-12~+12): " + (currentDate.get(Calendar.ZONE_OFFSET)/(60*60*1000))); 	// 서울시는 9시간 더 빠르다
