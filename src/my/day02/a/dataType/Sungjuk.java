...
public class Sungjuk {
	...
	String hakbun;	// null 학번 98134	201234	001234	091234
	...
	void sungjuk_print() {
		...
		short total = (short) (kor + eng + math); // 강제 형변환(casting) 0 ~ 300
		byte avg1 = (byte) (total / 3); // 10/4 => 2
										// 정수/정수 => 몫(정수)
		...
		double avg4 = (double)total /3; // 정수형 타입인 total 을 강제로 double 타입으로 형변환 시키는 것이다. 강제형변환(casting)
		float avg5 = (float)total /3;	// 정수형 타입인 total 을 강제로 float 타입으로 형변환 시키는 것이다. 강제형변환(casting)
		
		long money = 3000000000L;
		// 자바에서 정수는 기본적으로 int 타입을 취하므로 long 타입으로 나타내려면
		// 숫자 뒤에 L 또는 l(소문자 L)을 쓰면 된다.
	...
	static void boolean_print() {
		...
		// bool_1 = (10 == 10);		// 당연한 것을 물으니 아래 줄이 뜸
		System.out.println(bool_1); // true
	...
	static void char_print() {
		// 자바는 char 타입을 표현할때 unicode 를 사용한다.
		...
		System.out.println(ch1+0); // 65 char + int = int
		...
		char ch2 = 'a';
		System.out.println((char)(ch2-32)); // A 97 - 32 ==> 65(A)
		
		char ch3 = 9748;
		System.out.println(ch3); // 유니코드 16진수 계산기 hex(16) 클릭후 왼쪽 위쪽 더한 후 dec(10) 값 클릭
		
		char ch4 = 50577;		// 양
		char ch5 = 54812;		// 혜
		char ch6 = 51221;		// 정
		System.out.println(ch4+ch5+ch6); 		// int 값 : 156610
		System.out.println(""+ch4+ch5+ch6);		// 양혜정
