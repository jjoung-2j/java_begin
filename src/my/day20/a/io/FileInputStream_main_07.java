...
public class FileInputStream_main_07 {
	...
	public static void main(String args[]) {
		...
		System.out.println(">> 내용을 입력하세요 [입력하신 내용은 C:\\NCS\\iotestdata\\result.txt 파일에 저장됨.]");		// java에서 \\ 두개를 적어야 \ 이다.
		
		String fileName = "C:\\NCS\\iotestdata\\result.txt";	// C:/NCS/iotestdata/result.txt => java에서는 이것도 인식한다.
		...
		try {
			...
			FileOutputStream fost = new FileOutputStream(fileName, append);		// append 부분이 false 면 덧붙이기 X
			/*
				만약에 탐색기에서 
			...
			while((input = System.in.read()) != -1) {	// 프로그램 종료하기 전까지 계속
				fost.write(input);
				fost.flush();
...
/*
	C:\NCS\workspace(java)\Begin\bin>java my.day20.a.io.FileInputStream_main_07
	>> 내용을 입력하세요 [입력하신 내용은 C:\NCS\iotestdata\result.txt 파일에 저장됨.]
	superman 안녕하세요?
	Bye Bye
	^Z
	C:\NCS\iotestdata\result.txt에 쓰기 완료!! 31byte 씀
	반복횟수 : 31번 반복함
 */
