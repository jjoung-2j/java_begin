...
public class Main_Operator {
	...
	*/
		...
		bool = (c > d) && (d < e) && (c == e);	// && and : 거짓이 포함되면 거짓
				// F		T			F
				// false && 스킵
		System.out.println("bool => " + bool);	// bool => false
		...
		bool = (c > d) & (d < e) & (c == e);
				// F		T			F
				// false & trie & false => 스킵 X
		System.out.println("bool => " + bool);	// bool => false
		
		System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
		
		bool = (c > d) || (d < e) || (c == e);	// || or : 참이 포함되면 참
				// F		T			F
				// false || true || 스킵
		System.out.println("bool => " + bool);	// bool => true
		...
		System.out.println("\n~~~~~~~~ 퀴즈1 ~~~~~~~\n");
		int i = 1;		// i = 1
		int j = i++;	// int j = 1; j=i+1							// i = 2, j = 1
						// j = 1; i = 2; 
		
		if((i > ++j) & (i++ == j)) {	// f & t // j = 2			// i = 2 > j = 2 => F ==> i = 2, j = 2
			i = i + j;												
			/*														
				j = j+1; j ==> 2		// i = 2, j = 2 /			// &
				(i > j) false
				...
				(i == j) true			// i = 2 ,j = 2 /			// i = 2 == j == 2 => T ==> i = 3, j = 2
				(2 = 2)		
				i = i+1
		...
		if((i > ++j) && (i++ ==j)) {	// f						// i = 2 > j = 2 => F ==> i = 2, j = 2
			i = i + j;				
		}															// && F 나올시 바로 스킵! 
		...
		*/
		...
		no%=3; // no = no % 3; 와 같은 것이다.
		System.out.println("no = " + no);	// no = 2 (몫:0, 나머지:2)
		...
		System.out.println("num3 => " + num3);		// num 3 => 60
